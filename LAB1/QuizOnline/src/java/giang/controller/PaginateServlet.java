        ...
        try {
            ...
            if (subjectCode != null) {
                ...
                if (viewType.equals("Question")) {
                    ArrayList<TblQuestionDTO> dtoList = (ArrayList<TblQuestionDTO>) request.getAttribute("SEARCHRESULT");
                    request.setAttribute("SEARCHSIZE", Pagination.calculateNumofPage(dtoList.size(), ROW_PER_PAGE));
                    System.out.println(dtoList.size());
                    System.out.println(Pagination.calculateNumofPage(dtoList.size(), ROW_PER_PAGE));
                    ...
                    ArrayList<TblQuizDTO> dtoList = (ArrayList<TblQuizDTO>) request.getAttribute("SEARCHRESULT");
                    request.setAttribute("SEARCHSIZE", Pagination.calculateNumofPage(dtoList.size(), ROW_PER_PAGE));
                    ArrayList<TblQuizDTO> pagingResult = Pagination.getPagingQuiz(dtoList, ROW_PER_PAGE, currentPage + 1);
                    request.setAttribute("PAGINGRESULT", pagingResult);
                    request.setAttribute("CURRENTPAGE", currentPage + 1);
                    url = HISTORY_PAGE;
                }
